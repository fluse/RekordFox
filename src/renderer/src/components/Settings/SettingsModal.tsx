...
export default function SettingsModal({
  ...
  const handleUpdateHistoryLimit = async (historyLimit: number): Promise<void> => {
    if (historyLimit === settings.historyLimit) return
    try {
      await onUpdateSettings({ historyLimit })
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : String(err)
      setError(msg || t('settings.errorChangeHistoryLimit'))
    }
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="relative w-full max-w-md rounded-xl border border-zinc-800 bg-zinc-950 shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <h2 className="text-xl font-bold text-zinc-100 p-6 pb-3">{t('settings.title')}</h2>

        <div className="flex gap-1.5 border-b border-zinc-900 px-6 pb-3">
          {(
            [
            ...
            <>
              ...
              <div>
                <div className="flex justify-between items-center mb-2">
                  ...
                  >
                    {t('settings.historyLimitCount', { count: settings.historyLimit ?? 50 })}
                  </span>
                </div>
                <div className="flex items-center gap-4">
                  <input
                    ...
                    onChange={(e) => handleUpdateHistoryLimit(parseInt(e.target.value))}
                    disabled={loading}
                    className={`flex-1 h-2 cursor-pointer bg-zinc-900 rounded-lg outline-none border border-zinc-800 ${
                      settings.theme === 'light' ? 'accent-amber-600' : 'accent-primary'
                    }`}
